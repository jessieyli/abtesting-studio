...
def get_standard_error(a, b):
    ...
    n_a = len(a)
    n_b = len(b)
    return (get_stdev(a)**2/n_a + get_stdev(b)**2/n_b)**(1/2)
...
def get_2_sample_df(a, b):
    ...
    std_b = get_stdev(b)
    n_b = len(b)
    return round(se**4 / (((std_a**2/n_a)**2)/(n_a-1) + ((std_b**2/n_b)**2)/(n_b-1))) 
    ...
    ** DO NOT modify the parameter, observed_grid. **
    ...
    for i in range(num_rows):
        ...
        for j in range(num_cols):
            row_sum += observed_grid[i][j]
        row_sums.append(row_sum)
    col_sums = []
    for j in range(num_cols):
        col_sum = 0
        for i in range(num_rows):
            col_sum += observed_grid[i][j]
    ...
    for i in range(num_rows):
        ...
        for j in range(num_cols):
            expected_counts[i].append(row_sums[i]*col_sums[j] / total_sum)
...
def chi2_value(observed_grid):
    ...
    for i in range(len(observed_grid)):
        for j in range(len(observed_grid[0])):
            summation += (observed_grid[i][j]-expected_grid[i][j])**2 / expected_grid[i][j]
    return summation
...
def perform_chi2_homogeneity_test(observed_grid):
    ...
    ** DO NOT CHANGE THE NAME OF THIS FUNCTION!! ** (this will mess with our autograder)
    ...
    '''
    chi_value = chi2_value(observed_grid)
    df = df_chi2(observed_grid)
    return 1-chi2.cdf(chi_value, df)
...
grid_rowa = data_to_num_list(returnA)
grid_rowb = data_to_num_list(returnB)
return_grid = [grid_rowa, grid_rowb]
print("chi^2 val: ", chi2_value(return_grid)) 
print("p-val: ", perform_chi2_homogeneity_test(return_grid))
